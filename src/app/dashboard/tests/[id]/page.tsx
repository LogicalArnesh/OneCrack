    ...
    <div className="min-h-screen bg-background flex flex-col overflow-hidden">
        ...
        <AlertDialogContent className="rounded-[2rem] max-w-xl p-10 border-destructive/20 bg-card">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-center space-y-4 pt-2">
              <p className="font-medium text-foreground">By proceeding, you agree to take this test under strict evaluation standards. Use of external aids or unauthorized resources will result in automatic disqualification.</p>
              <p className="text-xs font-bold text-destructive uppercase tracking-widest mt-4">OneCrack Evaluator Monitoring Active</p>
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <header className="h-16 border-b border-border bg-card/80 backdrop-blur-md flex items-center justify-between px-6 z-50">
        ...
        <div className="flex items-center gap-6">
          ...
          <Button onClick={() => finishTest()} disabled={isSubmitting} className="rounded-xl font-bold bg-foreground text-background hover:bg-foreground/90 px-6 gap-2">
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="flex-1 overflow-y-auto p-4 md:p-10 relative">
          <div className="max-w-3xl mx-auto space-y-10">
            ...
            <div className="space-y-8">
              <h2 className="text-2xl font-headline font-bold leading-snug text-foreground/90">{currentQ.questionText}</h2>
              {currentQ.options && (
                <RadioGroup value={attempts[currentQ.id]?.selectedOption || ""} onValueChange={handleOptionSelect} className="grid grid-cols-1 gap-4">
                  {currentQ.options.map((option, i) => (
                    <Label key={i} className={cn(
                        "flex items-center gap-4 p-6 rounded-[1.5rem] border-2 transition-all cursor-pointer group shadow-sm",
                        attempts[currentQ.id]?.selectedOption === option ? "border-primary bg-primary/5" : "border-border bg-card/50 hover:border-primary/20"
                      )}>
                      <RadioGroupItem value={option} className="sr-only" />
                      <div className={cn(
                        "w-10 h-10 rounded-xl border-2 flex items-center justify-center font-black text-sm transition-all",
                        attempts[currentQ.id]?.selectedOption === option ? "bg-primary border-primary text-white" : "border-border text-muted-foreground"
            ...
            <div className="pt-10 flex flex-col sm:flex-row items-center justify-between gap-6 border-t border-border">
              <Button variant="outline" onClick={() => setCurrentIdx(prev => Math.max(0, prev - 1))} disabled={currentIdx === 0} className="rounded-xl px-6 h-12 font-bold">
                <ChevronLeft className="w-4 h-4 mr-2" /> Back
              </Button>
              <Button onClick={() => {
                  if (currentIdx < test.questions.length - 1) setCurrentIdx(currentIdx + 1);
                  else finishTest();
                }} className="w-full sm:w-auto rounded-xl px-10 h-12 font-black bg-primary text-white shadow-xl shadow-primary/20">
        ...
        <aside className="w-80 border-l border-border bg-card/50 backdrop-blur-sm hidden lg:flex flex-col p-6 space-y-8">
              ...
              {test.questions.map((q, i) => {
                ...
                return (
                  <button key={q.id} onClick={() => setCurrentIdx(i)} className={cn(
                      ...
                      status === 'attempted' ? "bg-primary text-white" : "bg-muted/10 text-muted-foreground"
                    )}>
                    {i + 1}
                  </button>
                );
          ...
          <div className="mt-auto p-5 rounded-2xl bg-destructive/5 border border-destructive/10">
            <div className="flex items-center gap-2 text-destructive font-black text-[10px] uppercase tracking-widest mb-2">
              <ShieldAlert className="w-4 h-4" /> Security Active
            </div>
            <p className="text-[9px] text-muted-foreground font-medium">Session recorded for evaluation audits. Malpractice will cancel result.</p>
          </div>
        </aside>
      </div>
    </div>
